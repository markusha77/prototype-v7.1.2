import { Code, Globe, Link as LinkIcon, Tag, Users } from 'lucide-react';
...
const COMMUNITIES = ['EcoTech Innovators', 'AI Creators Collective', 'Design Systems Guild'];
...
const ProjectForm: React.FC = () => {
  ...
  const existingProject =
    projectId && profile?.projects ? profile.projects.find((p) => p.id === projectId) : null;
  ...
  const handleImageSelect = (file: File, isMain: boolean = false) => {
    ...
    if (isMain && !formData.image) {
      ...
      setProjectImages((prev) => [...prev, { url: imageUrl, file, isMain: true }]);
    } else {
      // Add to additional images
      setProjectImages((prev) => [...prev, { url: imageUrl, file, isMain: false }]);
    }
  };
      ...
      if (mainImage) {
        ...
        if (remainingImages.length > 0) {
          ...
          setProjectImages((prev) =>
            prev.map((img) => (img.url === newMainImage.url ? { ...img, isMain: true } : img)),
          );
        } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const additionalImages = projectImages.filter((img) => !img.isMain).map((img) => img.url);
  ...
  const additionalImageUrls = projectImages.filter((img) => !img.isMain).map((img) => img.url);

  return (
    <div className="max-w-3xl mx-auto mt-8 mb-10 px-5 md:px-0">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  />
                  {errors.technologies && (
                    <p className="mt-1.5 text-sm text-red-600">{errors.technologies}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-100">
              <h3 className="text-lg font-medium text-gray-800 mb-2">
                Share with Communities & Open Spaces
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <label className="block text-sm font-medium text-gray-700">Open Spaces</label>
                  </div>
                  <MultiSelect
                    options={OPEN_SPACES}
              ...
              {(formData.selectedCommunities?.length || 0) +
                (formData.selectedOpenSpaces?.length || 0) >
                0 && (
                <div className="mt-4 p-3 bg-white rounded-lg border border-blue-200">
                  <div className="flex items-center gap-2 text-sm text-blue-700">
                    <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                    <span className="font-medium">
                      Your project will be shared with{' '}
                      {(formData.selectedCommunities?.length || 0) +
                        (formData.selectedOpenSpaces?.length || 0)}{' '}
                      location(s)
                    </span>
